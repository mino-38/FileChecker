...
class TaskTray:
    ...
    def start(self):
        options_map = {"Hide": self.root.withdraw, "Show": self.root.deiconify, "Quit": lambda: self.root.after(1, self.quit)}
        items = []
...
def create():
    ...
    scrollbar = tkinter.Scrollbar(root, orient=tkinter.VERTICAL, command=canvas.yview)
